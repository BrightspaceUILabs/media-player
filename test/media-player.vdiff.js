...
describe('d2l-labs-media-player', () => {
	it('video', async() => {
		const elem = await fixture(html`<d2l-labs-media-player src="./videos/1_lego.webm" media-type="video"></d2l-labs-media-player>`);
		await expect(elem).to.be.golden();
	});

	it('video-with-poster', async() => {
		const elem = await fixture(html`<d2l-labs-media-player src="./videos/1_lego.webm" poster="../demo/sample-poster.png" media-type="video"></d2l-labs-media-player>`);
		await expect(elem).to.be.golden();
	});

	it('audio', async() => {
		const elem = await fixture(html`<d2l-labs-media-player src="./audio/applause.mp3" media-type="audio"></d2l-labs-media-player>`);
		await expect(elem).to.be.golden();
	});
